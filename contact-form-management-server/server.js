...
app.get('/api/messages', async (req, res) => {
  ...
  const { page = 1, pageSize = 100, sortField = 'creationDate', sortOrder = 'desc' } = req.query;
  ...
  const paginatedMessages = sortedMessages.slice(startIndex, startIndex + pageSizeNum);
  res.status(200).send({
    data: {
    ...
    meta: {
      ...
      totalPages: Math.ceil(messages.length / pageSizeNum),
    },
  });
});
